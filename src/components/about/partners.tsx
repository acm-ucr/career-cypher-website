...
const Partners = () => {
  return (
    <div className="border-cypher-grey relative mt-30 flex h-fit w-1/2 flex-wrap items-center justify-center rounded-xl border-1 px-10 py-15 opacity-70 sm:py-20 md:w-3/4">
        ...
        {partners.map(({ name, image }, index) => (
          ...
          >
            <Image
              src={image}
              alt={`Logo: ${name}`}
              className="h-24 w-24 object-cover md:h-28 md:w-28 lg:h-30 lg:w-30"
            />
            <div>{name}</div>
          </div>
        ))}
